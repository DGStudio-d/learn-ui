...
export default function AdminMeetingsPage() {
  ...
  const upsertMut = useMutation({
    mutationFn: async () => {
      ...
      const res = await http.put<{ data: any }>(`/admin/meetings/${meetingId}/translations`, {
        locale,
        title,
        description,
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border rounded-xl p-4 max-w-xl">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-sm mb-1">Meeting ID</label>
            <input className="w-full border rounded px-3 py-2" value={meetingId} onChange={(e)=>setMeetingId(e.target.value)} placeholder="Enter meeting id" />
          </div>
          <div>
