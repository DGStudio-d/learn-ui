...
export default function ProgramQuizzesPage() {
  const { t, locale } = useI18n();
  const params = useParams<{ id: string }>();
  const programId = Number(params?.id);

  const quizzesQ = useQuery<QuizzesResponse>({
    queryKey: ["program", programId, "quizzes", locale],
    queryFn: async () => {
      const res = await http.get<QuizzesResponse>(`/programs/${programId}/quizzes`, {
        params: { with_questions: 0, locale },
      });
      return res.data;
    },
  ...
  return (
    <Container className="space-y-6 py-6">
      <div className="flex items-baseline justify-between gap-4">
        ...
        <Link href="/programs" className="text-sm text-green-700 hover:underline">{t("common.back_to_dashboard", "Back to dashboard")}</Link>
      </div>
          ...
          <Card key={q.id} className="flex flex-col">
            <CardContent className="p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between gap-2">
                <div className="font-semibold text-gray-900 truncate">{q.title || t("programs.quizzes", "Program Quizzes") + " #" + q.id}</div>
